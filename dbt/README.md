...
```bash
export DBT_PROFILES_DIR=.   # from this dbt/ directory
dbt debug
```
...
|---|---|
| `ingest` / `ingest-refresh` | `dbt run --select account_raw` (windowing is gated on `is_incremental()`, same as the other two models) |
...
- **`account_raw` currently uses dbt-trino's built-in `delete+insert` strategy**
  (`config(incremental_strategy='delete+insert')` in `account_raw.sql`), which
  rewrites every row in the windowed batch on every run, changed or not -
  including bumping `ingested_at` on rows whose `xmlrecord` didn't actually
  change. `macros/conditional_merge.sql` exists (a 1:1 port of `gen_sql.py`'s
  `mode_ingest_refresh` MERGE, only updating `WHEN MATCHED AND xmlrecord IS
  DISTINCT FROM`) but **is not wired to any model** - no `config()` in this
  project references the `conditional_merge` strategy name. Since
  `account_attributes` windows its own re-tokenization by
  `account_raw.ingested_at`, this means an unchanged row can still trigger a
  re-tokenize downstream. Known, currently-unaddressed gap between the macro's
  intent and what actually runs.
...
## Local stack note: `iceberg-rest`'s catalog is Postgres-backed

The `tabulario/iceberg-rest` image's catalog registry (table name -> current
`metadata.json` pointer) is backed by a dedicated `iceberg-catalog-db`
Postgres service in `docker-compose.yml`, via `CATALOG_URI:
jdbc:postgresql://iceberg-catalog-db:5432/${ICEBERG_CATALOG_DB_NAME:-iceberg_catalog}`,
persisted in the named volume `iceberg-catalog-pg`. The JDBC catalog
auto-creates its `iceberg_tables`/`iceberg_namespace_properties` tables on
first use - an empty result querying them right after `docker compose up -d`
just means nothing has been written to the catalog yet, not a broken setup.

## Validated

Full `dbt build` (seed + all 6 models + all 10 tests, `account` and
`customer` alike) passes clean against the local stack. Baselines match
`trino_parsing/README.md`'s documented, already-verified numbers exactly for
`account`:
