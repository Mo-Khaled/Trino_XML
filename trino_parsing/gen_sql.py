...
def load_lookup_csv(path, table):
    """Read (field_index, m_index, resolved_name_en) for one table from a CSV
    export of the lookup table. Blank m_index defaults to 1 -- same
    "T24 omits it for the first occurrence" convention used everywhere else
    in this pipeline, applied here to mean "the primary/first occurrence."
    """
    rows = []
...
def build_wide_columns(a):
    """Shared by --mode wide and --mode wide-incremental: (rows, columns_sql).

    Every lookup row already pins an exact (field_index, m_index) -- blank
    m_index defaults to 1 -- so this is a plain aggregate pivot: no arrays,
    no per-field cardinality discovery, unlike the old wide-column design.
    The lookup table is trusted as the complete, authoritative column list.
    """
    rows = load_lookup_csv(a.lookup, a.table)
    if not rows:
        sys.exit(f"no lookup rows for table '{a.table}' in {a.lookup}")

    def lookup_expr(tag, m):
        return f"element_at(f, '{tag}_{m}')"

    columns = ["  recid"]
    for tag, m, name in rows:
        col = '"' + name.replace('"', '""') + '"'
        if tag == "c0":
            columns.append(f"  recid AS {col}")
        else:
            columns.append(f"  {lookup_expr(tag, m)} AS {col}")
    columns.append(
        f"  TRY(CAST(date_parse({lookup_expr(a.watermark_field, 1)}, "
        f"'%Y%m%d') AS DATE)) AS source_updated_date"
    )
    return rows, ",\n".join(columns)


def wide_pivot_cte(source) -> str:
    """One map per recid in a single pass (keyed by "tag_m"), instead of every
    output column separately scanning and comparing field_index/m_index
    against all of that recid's rows -- same map-once, element_at-many
    pattern already used for tokenizing (see token_ctes), applied here to
    the pivot instead. Duplicate keys (a genuine s > 1 sub-value sharing the
    same tag+m) collapse to one arbitrary value -- this mode has no per-field
    array/sub-value handling, same limitation a plain CASE/MAX pivot would
    also have.
    """
...
def mode_wide(a) -> str:
    """Pivot {a.source} (the EAV attributes table) into one row per recid,
    one named column per lookup row -- bootstrap: rebuilds the whole table.

    Reads --lookup, a CSV export, rather than joining --lookup-table live:
    a wide table's column list has to be fixed at generation time regardless
    of how current the export is, so a live join wouldn't remove the need to
    regenerate -- see the module docstring and trino_parsing/README.md.
    """
    rows, columns_sql = build_wide_columns(a)

    return f"""-- Generated by gen_sql.py --mode wide --table {a.table}
-- One row per recid, one column per lookup row ({len(rows)} columns).
-- Every lookup row pins an exact (field_index, m_index); blank m_index
-- defaults to 1. NULL where a record's XML never had that tag/position.

CREATE SCHEMA IF NOT EXISTS {a.schema}
WITH (location = 's3://warehouse/bronze');

DROP TABLE IF EXISTS {a.wide_target};

CREATE TABLE {a.wide_target}
WITH (format = 'PARQUET') AS
{wide_pivot_cte(a.source)}
SELECT
{columns_sql},
  xml_hash,
  current_timestamp AS ingested_at
...
SELECT count(*) AS row_count FROM {a.wide_target};
"""


def mode_wide_incremental(a) -> str:
    """Re-pivot only recids whose account_attributes rows changed, then
    delete+insert just those into {a.wide_target} -- the wide-table
    equivalent of --mode incremental.

    --start-date/--end-date (same flags as ingest-refresh/incremental)
    window the source scan by account_attributes.ingested_at -- which
    --mode incremental only ever bumps for recids it actually changed -- so
    this never has to re-pivot the whole table. Like --mode incremental,
    the window is an optimization on what gets re-pivoted, not the
    correctness check: xml_hash IS DISTINCT FROM is what actually decides
    whether a recid gets replaced.
    """
    rows, columns_sql = build_wide_columns(a)

    if a.start_date:
        window_comment = (
            f"-- Window: only re-pivots {a.source} rows whose ingested_at falls in "
            f"[{a.start_date}, {a.end_date}] -- --mode incremental only bumps ingested_at "
            f"for recids it actually changed, so this must use the same window (or "
            f"wider) as the incremental run that fed this, or a change outside it "
            f"would be silently skipped here."
        )
        windowed_source = (
            f"(SELECT * FROM {a.source} "
            f"WHERE ingested_at >= date_parse('{a.start_date}', '%Y%m%d') "
        ...
        window_comment = f"-- No --start-date/--end-date given: re-pivots all of {a.source}."
        windowed_source = a.source

    return f"""-- Generated by gen_sql.py --mode wide-incremental --table {a.table}
{window_comment}
-- Prerequisite: run --mode wide once, and --mode incremental before this.

DROP TABLE IF EXISTS {a.wide_target}_stage;

CREATE TABLE {a.wide_target}_stage
WITH (format = 'PARQUET') AS
{wide_pivot_cte(windowed_source)}
SELECT
{columns_sql},
  xml_hash,
  current_timestamp AS ingested_at
...
DROP TABLE IF EXISTS {a.wide_target}_changed;

CREATE TABLE {a.wide_target}_changed
WITH (format = 'PARQUET') AS
SELECT DISTINCT s.recid
FROM {a.wide_target}_stage AS s
LEFT JOIN (SELECT DISTINCT recid, xml_hash FROM {a.wide_target}) AS t
  ON t.recid = s.recid
WHERE t.recid IS NULL OR t.xml_hash IS DISTINCT FROM s.xml_hash;

DELETE FROM {a.wide_target}
WHERE recid IN (SELECT recid FROM {a.wide_target}_changed);

INSERT INTO {a.wide_target}
SELECT s.*
FROM {a.wide_target}_stage AS s
JOIN {a.wide_target}_changed AS c ON c.recid = s.recid;

SELECT count(*) AS changed_records FROM {a.wide_target}_changed;
...
def main():
    ...
    ap.add_argument("--mode", default="bootstrap",
                    choices=["ingest", "ingest-refresh", "bootstrap", "incremental",
                             "reconcile", "wide", "wide-incremental"])
    ...
    ap.add_argument("--lookup", default=str(Path(__file__).resolve().parent.parent
                    ...
                    help="--mode wide/wide-incremental only: CSV export of the lookup "
                         "table. Not a live connection -- the wide table's column list "
                         "has to be fixed at generation time regardless, so this is "
                         "meant to be a fresh export (e.g. by an Airflow task) each "
                         "time these modes run, not a hand-maintained file")
    ap.add_argument("--wide-target", default=None,
                    help="--mode wide/wide-incremental only: output table "
                         "(default: <schema>.<table>_wide)")
    ...
    # wide/wide-incremental pivot the already-parsed attributes table, not raw XML.
    a.source = a.source or (
        a.target if a.mode in ("wide", "wide-incremental") else a.raw_target
    )
    ...
    sql = {
        ...
        "wide": mode_wide,
        "wide-incremental": mode_wide_incremental,
    }[a.mode](a)
